...
output_root <- file.path("simstudy_runs_mc", format(Sys.time(), "%Y%m%d_%H%M%S"))
...
# -----------------------------------------------------------------------------
# Assemble task list per Monte Carlo run
# -----------------------------------------------------------------------------

assemble_task_suite <- function(seed) {
  if (!missing(seed) && !is.null(seed)) set.seed(seed)
  tasks <- list()
  tasks <- append(tasks, create_univariate_tasks(generate_isolated(),
                                                base_id = "uni_isolated",
                                                base_title = "Univariate: Isolated",
                                                K_init = 8L,
                                                min_size = 3L,
                                                min_prop = 0.05))
  tasks <- append(tasks, create_univariate_tasks(generate_mag1(),
                                                base_id = "uni_mag1",
                                                base_title = "Univariate: Mag1",
                                                K_init = 8L,
                                                min_size = 3L,
                                                min_prop = 0.05))
  tasks <- append(tasks, create_univariate_tasks(generate_mag2(),
                                                base_id = "uni_mag2",
                                                base_title = "Univariate: Mag2",
                                                K_init = 8L,
                                                min_size = 3L,
                                                min_prop = 0.05))
  tasks <- append(tasks, create_univariate_tasks(generate_shape(),
                                                base_id = "uni_shape",
                                                base_title = "Univariate: Shape",
                                                K_init = 8L,
                                                min_size = 3L,
                                                min_prop = 0.05))
  tasks <- append(tasks,
                  list(create_mv_task(generate_mv_one(), id = "mv_one_channel",
                                      title = "MV: Single Channel", K_init = 5L,
                                      min_size = 2L, min_prop = 0.05)))
  tasks <- append(tasks,
                  list(create_mv_task(generate_mv_two(), id = "mv_two_channels",
                                      title = "MV: Two Channels", K_init = 5L,
                                      min_size = 2L, min_prop = 0.05)))
  tasks <- append(tasks,
...
run_dataset_task <- function(task) {
  ...
  message(sprintf(
    "[run %03d | %s] Starting wicmad() run (n_iter=%d, burn=%d, thin=%d, K_init=%d)",
    task$run_id, task$dataset_id, iterations, burnin, thin, task$K_init
  ))
  ...
  run_model <- function() {
    wicmad(
      Y = task$Y,
      t = task$t,
      mean_intercept = model_config$mean_intercept,
      n_iter = iterations,
      burn = burnin,
      thin = thin,
      K_init = task$K_init,
  ...
  res <- with_kernel_bias_mode(model_config$kernel_mode, run_model())
...
# -----------------------------------------------------------------------------
# Assemble jobs across Monte Carlo runs
# -----------------------------------------------------------------------------

jobs <- list()
job_idx <- 1L
for (run_id in seq_len(mc_runs)) {
  run_seed <- base_seed + run_id - 1L
  tasks <- assemble_task_suite(run_seed)
  for (task in tasks) {
    task$run_id <- run_id
    task$run_seed <- run_seed
    task$dataset_id <- task$id
    task$dataset_label <- task$display_name
    task$dataset_dir <- file.path(output_root, task$dataset_id)
    task$output_dir <- file.path(task$dataset_dir, sprintf("run_%03d", run_id))
    jobs[[job_idx]] <- task
    job_idx <- job_idx + 1L
  }
}
...
dataset_count <- length(assemble_task_suite(base_seed))
message(sprintf(
  "Monte Carlo runs per dataset: %d. Total datasets per run: %d. Total jobs: %d.",
  mc_runs, dataset_count, total_jobs
))
...
worker_count <- min(target_workers, available_cores)
if (worker_count < target_workers) {
  warning(sprintf("Only %d cores detected; using %d workers instead of requested %d.",
                  available_cores, worker_count, target_workers))
}

if (worker_count <= 1L) {
  message(sprintf("Running %d Monte Carlo dataset fits (sequential mode)...", total_jobs))
  ...
  for (idx in seq_along(jobs)) {
    results[[idx]] <- run_dataset_task(jobs[[idx]])
    progress$tick()
  }
  progress$finish()
} else {
  message(sprintf(
    "Running %d Monte Carlo dataset fits across %d workers...",
  ...
  parallel::clusterExport(cl,
    varlist = c("run_dataset_task", "iterations", "burnin", "thin", "warmup_iters",
                ...
                "determine_normal_anomaly_clusters", "with_kernel_bias_mode",
                "model_config"),
    envir = environment()
  )
...
message("All Monte Carlo runs completed.")
