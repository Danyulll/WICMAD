...
load_coffee_data <- function(data_dir, imbalance_ratio = 0.10) {
  ...
  cat("Normal (Class 0):", sum(imbalanced_labels == 0), "\n")
  cat("Anomaly (Class 1):", sum(imbalanced_labels == 1), "\n")
...
perform_fpca <- function(features, target_dim = 16) {
  cat("Performing functional PCA analysis...\n")
  
  # Find the highest power of 2 supported by the data
  max_dim <- ncol(features)
  p_dim <- 2^floor(log2(max_dim))
  cat("Using p parameter for FPCA (highest power of 2):", p_dim, "\n")
  
  # Interpolate all series to same length
  n_series <- nrow(features)
  interpolated_features <- matrix(0, nrow = n_series, ncol = p_dim)
  
  for (i in seq_len(n_series)) {
    interpolated_features[i, ] <- interpolate_series(features[i, ], p_dim)
  }
...
prepare_wicmad_data <- function(features, labels) {
  ...
  # Interpolate each time series to p dimensions
  n_series <- nrow(features)
  interpolated_features <- matrix(0, nrow = n_series, ncol = p_dim)
  ...
  # For univariate data, we need P=p_dim (time points) and M=1 (channels)
  Y <- lapply(seq_len(n_series), function(i) {
    matrix(interpolated_features[i, ], nrow = p_dim, ncol = 1)
  })
...
plot_overlapped_data <- function(features, labels, title = "Coffee - Before Clustering", max_series = NULL) {
...
plot_original_data <- function(features, labels, max_series = NULL) {
  return(plot_overlapped_data(features, labels, "Coffee - Before Clustering", max_series))
}

# Function to plot clustering results (overlapped)
plot_clustering_results <- function(features, true_labels, cluster_assignments, title = "Coffee - After Clustering", max_series = NULL) {
...
plot_derivative_clustering_results <- function(original_data, first_deriv_data, second_deriv_data, cluster_assignments, title = "Coffee - After Clustering", max_series = NULL) {
...
plot_fpca_clustering_results <- function(fpca_result, true_labels, cluster_assignments, title = "Coffee - After Clustering", max_series = NULL) {
  ...
  p <- ggplot(plot_data, aes(x = .data$time, y = .data$value, color = .data$cluster_name, group = .data$series_id)) +
    ...
    labs(title = title,
         x = "Time", y = "Component Value", color = "Class") +
    scale_color_manual(values = c("Normal" = "blue", "Anomaly" = "red")) +
    theme_minimal() +
    theme(legend.position = "bottom")
...
prepare_derivative_data <- function(features, labels) {
  ...
  for (i in seq_len(n_series)) {
    original_interp[i, ] <- interpolate_series(features[i, ], p_dim)
  }
...
prepare_fpca_data <- function(features, labels) {
  ...
  # Perform functional PCA
  fpca_result <- perform_fpca(features)
