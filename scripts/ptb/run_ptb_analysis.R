...
plot_overlapped_data <- function(series_list, labels, title = "PTB - Before Clustering", max_series = NULL) {
  ...
  # Create ordered factor for proper lead ordering
  plot_data$dimension <- factor(plot_data$dimension, 
                                 levels = paste("Lead", sort(unique(plot_data$dimension_order))))
...
plot_clustering_results <- function(series_list, true_labels, cluster_assignments, title = "PTB - After Clustering", max_series = NULL) {
