...
prepare_wicmad_data <- function(series_list, labels) {
  ...
  # P=p_dim (time points), M=6 (dimensions)
...
plot_overlapped_data <- function(series_list, labels, title = "BasicMotions - Before Clustering", max_series = NULL) {
...
plot_clustering_results <- function(series_list, true_labels, cluster_assignments, title = "BasicMotions - After Clustering", max_series = NULL) {
...
main <- function() {
  ...
  # (This is already handled in the load_basicmotions_data function)
