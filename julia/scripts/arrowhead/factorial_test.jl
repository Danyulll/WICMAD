...
function run_single_experiment(exp_id::Int, params::Dict, data_dir::String, results_file::String)
    try
        ...
        result = wicmad(prepped.series, t;
            n_iter = 50,  # Very reduced for testing
            burn = 25,
            thin = 1,
...
# Function to create a smaller parameter grid for testing
function create_test_parameter_grid()
    # Reduced parameter levels for testing
    alpha_prior_levels = [(1.0, 1.0), (10.0, 1.0)]
    kappa_pi_levels = [0.1, 0.6]
    c2_levels = [0.5, 1.0]
    tau_pi_levels = [10, 40]
    reveal_ratio_levels = [0.15, 0.45]
    warmup_iters_levels = [100, 500]
...
function main()
    ...
    results_file = joinpath(@__DIR__, "factorial_test_results.toml")
    ...
    parameter_combinations = create_test_parameter_grid()
    total_experiments = length(parameter_combinations)
    
    println("Factorial Test Experiment Setup")
    println("="^50)
